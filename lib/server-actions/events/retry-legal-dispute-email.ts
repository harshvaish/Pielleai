...
export async function retryLegalDisputeEmail(
  eventId: number,
  ...
  try {
    ...
    const validation = retrySchema.safeParse({ eventId });
    ...
    const [eventRow] = await database
      .select({
        ...
        cancellationRequestedBy: events.cancellationRequestedBy,
        cancellationNotes: events.cancellationNotes,
        cancellationLegalEmailSentAt: events.cancellationLegalEmailSentAt,
        availability: {
    ...
    if (eventRow.status !== 'in-dispute' || eventRow.cancellationType !== 'legal-dispute') {
      throw new AppError('Evento non in contestazione legale.');
    }

    if (eventRow.cancellationLegalEmailSentAt) {
      throw new AppError('Email legale gia inviata.');
    }
    ...
    const requestedByLabel = eventRow.cancellationRequestedBy
      ? REQUESTED_BY_LABELS[eventRow.cancellationRequestedBy] ?? eventRow.cancellationRequestedBy
      : 'Booking';
    ...
    const text = `Contestazione legale evento\n\n` +
      ...
      `Motivazione: ${eventRow.cancellationNotes ?? '-'}\n`;
    ...
    const html = `
      ...
      <p>${escapeHtml(eventRow.cancellationNotes ?? '-').replace(/\n/g, '<br/>')}</p>
    `;
    ...
    console.error('[retryLegalDisputeEmail] - Error:', error);
    return {
      success: false,
      message:
        error instanceof AppError
          ? error.message
          : 'Invio email legale non riuscito.',
      data: null,
    };
  }
}
