...
export async function POST(
  ...
  try {
    ...
    if (!session || (session.user.role !== "admin" && session.user.role !== "artist-manager")) {
    ...
    const validation = artistDocumentUploadSchema.safeParse(body);
    ...
    const { name, artistId } = validation.data;
    const sanitizedFileName = sanitizeFileName(name);
    const filePath = `artist-documents/${artistId}-${Date.now()}-${sanitizedFileName}`;
